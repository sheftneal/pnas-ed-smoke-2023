...
##################################################################                      
        ...
        ######### panel 1: Google Searches ##############
          ...
          goog = goog %>% mutate(smokePM_pred = smokePM,
                                 ...
                                 regbin4 = bin25_50 ,  #90-95 percentile
                                 regbin5 =  bin50_100 + bin100 #95+ percentile
          )
          
          
          # Binned model
          model = feols(hits~ regbin1 + regbin2 + regbin3 + regbin4 + regbin5 + temperature+ precipitation | dmamonth+date, goog) 
          

          df <- data.frame(bin = 1:5, coef = coef(model)[1:5], se = se(model)[1:5]) %>% mutate(lb = coef + qnorm(0.025)*se, ub = coef + qnorm(0.975)*se)
          df$baserate = mean(goog$hits, na.rm = T)
          write_rds(df, file = "data/fig5-panela-coefficients.rds")
        ...
        #########  panel 2: TRAFFIC ###############################
          ...
          #check max - min lat + lon to make sure no big moves over time
          
          data <- left_join(sdf_data, smd_data)
          write_rds(data, file = "data/caltran_station_daily_traffic_volume_2006_2017.rds", compress = "gz")
          ...
          #filter to stations with at least 90% obs non-missing over the 4383 period study period
          obs_count = data %>% group_by(stationid) %>% summarise(nobs = n())
          data <- left_join(data, obs_count)
          data <- filter(data, nobs > .9*4383) 
          ...
          #read in smoke PM grid from Childs et al
          spm_grid <- read_sf("data/10km_grid_wgs84.shp")
          ...
          tmp <-  read_rds('data/zip_day_prism.rds') 
          grid <- raster("data/PRISM_tmax_stable_4kmD2_20071214_bil.bil"); grid[] <- 1:ncell(grid);
          ...
          write_rds(data, file = "data/fig5_caltran_analysis_data.rds", compress ="gz")
          
          
          
          
          data <- read_rds("data/fig5_caltran_analysis_data.rds")
          ...
          #have parallel dataset for estimating only within cities
          data2 = filter(data, incity == 1) 
          
          vstat_locs <- data2 %>% dplyr::select(stationid, lon, lat) %>% distinct()
          
          #run different versions of model with/without lag and full sample/cities sample
          ...
          model2_lag <- feols(totflow ~ l(regbin1,0:7) + l(regbin2,0:7) + l(regbin3,0:7) + l(regbin4,0:7) + l(regbin5,0:7)+ tmx + ppt | stationid  + county_mth + doy + dow + year, data = data, cluster = "stationid", panel.id = c("stationid","date"))
          ...
          b1 = get_bin_response_cumulative(model2_lag, "regbin1",7,"stationid")
          b2 = get_bin_response_cumulative(model2_lag, "regbin2",7,"stationid")
          b3 = get_bin_response_cumulative(model2_lag, "regbin3",7,"stationid")
          b4 = get_bin_response_cumulative(model2_lag, "regbin4",7,"stationid")
          b5 = get_bin_response_cumulative(model2_lag, "regbin5",7,"stationid")
          ...
          write_rds(df, file = "data/fig5-panelb-coefficients.rds")
        ...
        ######### panel 3:  National Park Visitations ##############
                ...
                #process smoke PM data for NPs         
                  ...
                  #define relevant variables prior to analysis    
                    ...
                    tmp <-  read_rds('data/zip_day_prism.rds') 
                    grid <- raster("PRISM_tmax_stable_4kmD2_20071214_bil.bil"); grid[] <- 1:ncell(grid);
                    ...
                    #we only have data for zipcodes and some national parks not in zips so match to nearest zipcode to get data
              ...
              #estimate models  
                ...
                write_rds(df, file = "data/clean/fig5-panelc-coefficients.rds")
  ...
  ##################################################################                      
      ...
      pdf("figures/raw/fig-raw.pdf", width = 12, height = 4)
              
              par(mfrow = c(1,3))
              
              #panel a
              df = read_rds("data/clean/fig5-panela-coefficients.rds")
              ...
              #panel b
              df = read_rds("data/clean/fig5-panelb-coefficients.rds") %>% mutate(coef = coef/baserate, lb = lb/baserate, ub = ub/baserate)
              ...
              #panel c
              df = read_rds("data/clean/fig5-panelc-coefficients.rds") 
